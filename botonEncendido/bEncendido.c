...
#define LED_PIN_6 6        // Define el pin del LED
#define BUTTON_PIN_7 7     // Define el pin del botón
...
void inicializar_boton_encendido() {
    ...
    gpio_init(LED_PIN_6);   // Inicializa el pin del LED
    gpio_set_dir(LED_PIN_6, GPIO_OUT);   // Establece el pin del LED como salida
    gpio_init(BUTTON_PIN_7);   // Inicializa el pin del botón
    gpio_set_dir(BUTTON_PIN_7, GPIO_IN);   // Establece el pin del botón como entrada
    gpio_pull_up(BUTTON_PIN_7);   // Habilita la resistencia pull-up en el pin del botón
}

// Función para verificar si se presiona el botón de encendido
bool verificar_boton_encendido() {
    return gpio_get(BUTTON_PIN_7) == 0;   // Retorna verdadero si el botón está presionado (estado bajo)
}

// Función para controlar el LED de encendido
void control_leds_encendido() {
    if (!led_encendido) {   // Si el LED está apagado
        gpio_put(LED_PIN_6, 1);   // Enciende el LED
        ...
        gpio_put(LED_PIN_6, 0);   // Apaga el LED
